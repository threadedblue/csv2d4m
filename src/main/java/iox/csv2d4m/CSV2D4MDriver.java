...
//
// Reads CSV files and inserts them into D4M tables.  We assume that the CSV files have the first row as 
// the column headers and are well formed. i.e. The number of columns names equals the number of delimited fields 
// and all lines have the same number of delimited fields.
//
public class CSV2D4MDriver implements Runnable {

	private static final Logger log = LoggerFactory.getLogger(CSV2D4MDriver.class);
	...
	public CSV2D4MDriver(String[] args) throws CmdLineException {
		super();
	...
	public void run() {
		...
		try {
			...
			job.setJarByClass(CSV2D4MDriver.class);
			...
			job.setMapperClass(CSV2D4MMapper.class);
	...
	public static void main(String[] args) {
		try {
			CSV2D4MDriver app = new CSV2D4MDriver(args);
			log.info("Start==>");
			app.run();
			log.info("<==Finish");
		} catch (CmdLineException e) {
